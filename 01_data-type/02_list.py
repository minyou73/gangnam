...
# 일반적인 프로그래밍 언어와 다르게 길이를 동적으로 조절할수 있어 list 라고 부른다.
...
# 2. index(): 인자로 준 값이 몇번쨰 인덱스에 존재하는지 확인하여 그 인덱스를 반환한다.
# 같은 값이 리스트 내에 여러 개 존재하면 가장 처음에 존재하는 값의 인덱스를 반환한다.
print("index: ", fruits.index('apple'))

print("index: ", fruits.index('apple', 3))

# 3. reverse(): list값을 역으로 정렬한다. 원본 List에 영향을 준다.
fruits.reverse()
print(fruits)

# 4. append(): list끝에 값을 덧붙여 추가한다.
