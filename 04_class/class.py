...
# 메소드: 클래스 내부에 정의된 함수로, 인스턴스의 데이터를 조작하거나 동작을 정의한다.
    ...
    #       객체를 통해 접근시 호출되는 메소드이 첫번쨰 인자로 항상 self를 넘겨주어야 한다.
...
class Person:
    ...
    def information(self):
        return "i', from" + self.national + "and i use" + self.language

    def favorite(self, color):
        return "i love" + color
    

# 생성자 : __init__ : 객체가 생성될 때 자동으로 호출된다. 매개변수를 전달받아 인스턴스 속성을 초기화할수 있다.
...
# global
# 함수 내부에서 전역 변수를 참조하거나 수정할때  이용하며, 함수 내부에서 전역변수에 접근할 수 있다.
# 설정 값을 전역적으로 유지하고 여러 함수에서 이 값을 변경하거나 참조할때 유용하다.
...
# nonlocal
# 중첩 함수에서 바깥 함수의 변수를 참조하거나 수정할때 사용한다.
...
class BankAccount:
    def __init__(self, account_number, balance):
        ...
        self.__balance = balance  ## 이 변수는 직접접근 하지마!
...
# 비공개 속성에 직접 접근 시도
# __(언더스코어2개)로 시작하는 속성은 완전한 의미의 private 는 아니지만, 어느 정도 보호되는 속성으로 취급함.
# 이를 이름 장식(name managing)이라고 함
